...
This part explains how to set up and run Streamlit-based webcam application of this object detection project.
...
you can choose it from the dropdown menu available in the interface, 
or change the model path in the webcam-script.py before running.
...
In the [models](models/) directory those three formats are available, 
but here only the use of `.pt` format is shown. Streamlit doesn't support 
...
Press **Ctrl + C** to stop the server In the terminal where Streamlit is running. 
